...
namespace Voxelity.Extensions
{
    ...
    public static class TransformExtensions
    {
        ...
        public static List<Transform> GetChilds(this Transform parent, bool recursive = false)
        {
            List<Transform> gettedChilds = new List<Transform>();

            if (!recursive)
            {
                for (int i = 0; i < parent.childCount; i++)
                {
                    gettedChilds.Add(parent.GetChild(i));
                }
            }
            else
            {
        ...
        public static List<GameObject> GetChildGO(this Transform parent, bool recursive = false)
        {
            ...
            if (!recursive)
            {
                for (int i = 0; i < parent.childCount; i++)
                {
                    childGOs.Add(parent.transform.GetChild(i).gameObject);
                }
            }
            else
            {
                foreach (GameObject item in parent.GetComponentsInChildren<GameObject>())
                {
        ...
        public static void DestroyChilds(this Transform parent) => parent.GetChildGO().ForEach(GameObject.Destroy);
    }
}